...
const trimRight = (str: string, remove: string) => (str ? (str.endsWith(remove) ? str.slice(0, -1) : str) : '');

export const generateUrl = (protocol: string, domain: string, port: number | string, ...path: string[]) => {
